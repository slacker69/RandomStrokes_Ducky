...
void process_frame(uint16_t framenumber)
{
	...
	switch(state) {
		...
		case state_INJECTING:				
			do{
				injectToken = (rand() % (HID_KEYPAD_0 - HID_A)) + HID_A;
			}while(
			  injectToken == HID_CAPS_LOCK || // Block these Random Key Presses
			  injectToken == HID_SCROLL_LOCK || 
			  injectToken == HID_PRINTSCREEN || 
			  injectToken == HID_KEYPAD_NUM_LOCK || 
			  (injectToken >= HID_F1 && injectToken <= HID_F12)
			);
